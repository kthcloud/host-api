...
async function getRamLoad() {
  return si.mem().
    then(rawResult => {
      const load = Math.round(parseFloat(rawResult.active) / parseFloat(rawResult.total) * 100.0)
      return {
        main: load,
      }
    })
...
async function getNetworkUsage() {
  return si.networkStats()
    .then(rawResult => {
      ...
      return {
        receiveRate: rawResult[0].rx_sec == null ? 0 : Math.round(rawResult[0].rx_sec / seconds),
        transmitRate: rawResult[0].tx_sec == null ? 0 : Math.round(rawResult[0].tx_sec / seconds)
      }
    })
}
...
async function getGpuTemp() {
  return getSensorData()
    .then(sensorsData => {
      const gpuSensors = Object.values(sensorsData).filter(sensor => sensor.Adapter === 'PCI adapter')
